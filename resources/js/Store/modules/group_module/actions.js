...
const actions = 
{
    ...
    {
        axios.get(`chat/group/${state.id}/remove/${participant_id}`).then((res)=>{
    ...
    registerEventListeners({ state, rootState, commit, dispatch, getters, rootGetters }){
        Echo.private("group." + state.id )
            ...
            .listen('.participant.added', e => {
                ...
                isAmongThem
                    ? null // @todo console.log("error, .participant.added event - user listens for event which he shouldn't be listening to since he was no longer participant in chat group.")  
